...
const DatesModel = ({
  ...
  useEffect(() => {
    for (let i = 0; i < state.length; i++) {
      ...
      console.log('start date',moment(state[i].startDate).format("D/M/YY"));
      console.log('end date',state[i].endDate ? moment(state[i].endDate).format("D/M/YY") :'--');
      console.log('end date',state[i].endDate);
      console.log('end date',state[i].endDate=='Invalid date' ? '--':'not');
